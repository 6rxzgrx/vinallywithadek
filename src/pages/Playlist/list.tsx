...
export const PlaylistList: FC<PlaylistListProps> = memo(({ playlist }) => {
  ...
  // Filter songs to ensure they belong to this playlist's filters
  const songs = playlist.songs.filter((song) => {
    if (order === 'ALL') {
      // When showing all, ensure song types match playlist filters
      if (playlist.filters && playlist.filters.length > 0) {
        return song.types?.some(type => playlist.filters?.includes(type));
      }
      return true;
    }

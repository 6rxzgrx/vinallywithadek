...
export const PlaylistControls: FC<{ playlist: Playlist }> = ({ playlist }) => {
	...
	const handleSubmit = async (values: { name: string; wish: string }) => {
		...
		try {
			...
			message.error(
				error?.message || t('Failed to submit wish. Please try again.'),
			);
		} finally {
			setLoading(false);
		}
					...
					<Space>
						<Tooltip title={t('Filter')}>
							<Dropdown
								...
								menu={{
									items,
									selectedKeys: [order].filter((o) => o !== 'ALL'),
								}}
							>
								<button className="order-button">
									<Space align="center">
										<span
											style={{
												color: order !== 'ALL' ? 'inherit' : 'transparent',
											}}
										>
											{tor(order)}
										</span>
			...
			{isWeddingWishPlaylist && (
				...
				>
					...
					>
						...
						>
							<Input.TextArea rows={4} placeholder={t('Enter your wish')} />
						</Form.Item>
